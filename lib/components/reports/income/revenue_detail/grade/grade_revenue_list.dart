...
class GradeRevenueList extends StatefulWidget {
  String type,departmentId;


  GradeRevenueList(this.type,this.departmentId);

  @override
  _GradeRevenueListState createState() => _GradeRevenueListState();
}


class _GradeRevenueListState extends State<GradeRevenueList> {
      ...
      child: Column(
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection('grades').where("departmentId",isEqualTo: widget.departmentId).snapshots(),
  ...
  Future<int> getTotalRevenue(String valueType,String id)async{
    int total=0;
    await FirebaseFirestore.instance.collection('fees').where("gradeId",isEqualTo: id).where("feeCategory",isEqualTo:widget.type).get().then((QuerySnapshot querySnapshot) {
  ...
  DataRow _buildListItem(BuildContext context, DocumentSnapshot data) {
    final model = GradeModel.fromSnapshot(data);
    return DataRow(
        cells: [
          ...
          DataCell(
            InkWell(
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => ClassReportScreen(widget.type, model.id)));
