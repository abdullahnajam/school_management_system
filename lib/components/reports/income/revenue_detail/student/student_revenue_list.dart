...
class StudentRevenueList extends StatefulWidget {
  String type,classId;


  StudentRevenueList(this.type,this.classId);

  @override
  _StudentRevenueListState createState() => _StudentRevenueListState();
}


class _StudentRevenueListState extends State<StudentRevenueList> {
  ...
  Future<int> getTotalRevenue(String valueType,String id)async{
    int total=0;
    await FirebaseFirestore.instance.collection('fees').where("studentId",isEqualTo: id).where("feeCategory",isEqualTo:widget.type).get().then((QuerySnapshot querySnapshot) {
