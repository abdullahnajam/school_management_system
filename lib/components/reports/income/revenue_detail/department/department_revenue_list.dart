...
class DepartmentRevenueList extends StatefulWidget {
  String type,schoolId;


  DepartmentRevenueList(this.type,this.schoolId);

  @override
  _DepartmentRevenueListState createState() => _DepartmentRevenueListState();
}


class _DepartmentRevenueListState extends State<DepartmentRevenueList> {
      ...
      child: Column(
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection('departments').where("schoolId",isEqualTo: widget.schoolId).snapshots(),
  ...
  Future<int> getTotalRevenue(String valueType,String id)async{
    int total=0;
    await FirebaseFirestore.instance.collection('fees').where("departmentId",isEqualTo: id).where("feeCategory",isEqualTo:widget.type).get().then((QuerySnapshot querySnapshot) {
    ...
    return DataRow(
        cells: [
          ...
          DataCell(
            InkWell(
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => GradeReportScreen(widget.type, model.id)));
