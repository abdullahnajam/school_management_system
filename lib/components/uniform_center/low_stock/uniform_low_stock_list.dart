...
class UniformLowStockItemList extends StatefulWidget {
  const UniformLowStockItemList({Key? key}) : super(key: key);

  @override
  _UniformLowStockItemListState createState() => _UniformLowStockItemListState();
}


class _UniformLowStockItemListState extends State<UniformLowStockItemList> {

  Future<List<UniformItemModel>> getDocuments() async {
    List<UniformItemModel> items=[];
    await FirebaseFirestore.instance.collection('uniform_items').get().then((QuerySnapshot querySnapshot) {
      querySnapshot.docs.forEach((doc) {
        ...
        if(model.stock<=int.parse(model.lowStockQuantity))
          items.add(model);
      });
    });
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          FutureBuilder<List<UniformItemModel>>(
              future: getDocuments(),
                        ...
                        rows: List<DataRow>.generate(
                            snapshot.data!.length,
                                (index) => DataRow(cells: [
                                  ...
                                  DataCell(Text(snapshot.data![index].lowStockQuantity)),
                                  DataCell(Text(snapshot.data![index].price.toString())),
                                  DataCell(Text(snapshot.data![index].stock.toString())),
                                  DataCell(Text(snapshot.data![index].sku.toString())),
                                  DataCell(Image.network(snapshot.data![index].image)),
                            ])));
                  }
                }
              }
